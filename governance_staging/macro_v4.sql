...
with DAG(
    ...
    run_macro_scm_apply_tags = DbtRunOperationOperator(
        ...
        args={
            # Mandatory — KeyError at runtime if not passed, clear failure
            "db_name":     "{{ dag_run.conf['db_name'] }}",
            "schema_name": "{{ dag_run.conf['schema_name'] }}",
            "table_name":  "{{ dag_run.conf['table_name'] }}",
            # Optional — empty string when not passed, macro treats as none
            "since_date":  "{{ dag_run.conf.get('since_date', none) }}",
        },
