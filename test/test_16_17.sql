...
--------------------------------------------------------------------------------
-- Defining current timestamp (used for the look_back window)
--------------------------------------------------------------------------------
...
WITH source_raw AS (
    ...
    SELECT
        {% for col in key_columns %}
            COALESCE(
                {{ source_json_column }}:{{ afterState }}:{{ source_key_paths[loop.index0] }},
                {{ source_json_column }}:{{ beforeState }}:{{ source_key_paths[loop.index0] }}
            )::NUMBER AS {{ col }},
        {% endfor %}
        {{ source_json_column }}:metadata:{{ source_time_path }}::TIMESTAMP_NTZ AS metadata_time
    FROM {{ source_relation }}
    WHERE
        {% for col in key_columns %}
            {{ source_json_column }}:{{ afterState }}:{{ source_key_paths[loop.index0] }} IS NOT NULL
            AND
        {% endfor %}
        {{ source_json_column }}:metadata:{{ source_time_path }}::TIMESTAMP_NTZ
            >= dateadd(day, -{{ look_back }}, '{{ current_ts }}'::TIMESTAMP_NTZ)
...
{% if execute %}
    ...
    {{ log("[SCD2 Check] Failing rows found: " ~ fail_count, info=True) }}

    {% if fail_count > 0 %}
        {{ log("Connect with data team for refresh", info=True) }}
        {% if raise_error %}
            {{ exceptions.raise_compiler_error(
                "SCD2 validation FAILED for " ~ target_relation ~ " - "
                ~ fail_count ~ " failing row(s) found in the last " ~ look_back ~ " day(s). Check dbt.log for details."
            ) }}
        {% else %}
            {{ log("Warning: SCD2 validation failed but continuing (raise_error=False).", info=True) }}
        {% endif %}
    {% else %}
        {{ log("SCD2 validation PASSED for " ~ target_relation ~ " (last " ~ look_back ~ " day(s))", info=True) }}
    {% endif %}
