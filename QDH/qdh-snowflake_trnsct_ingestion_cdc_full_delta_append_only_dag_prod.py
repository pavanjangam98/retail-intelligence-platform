...
# =============================================================================
# DAG Definition - Flow for APPEND vs FULL/DELTA
# =============================================================================
with DAG(
    dag_id='qdh-snowflake_trnsct_ingestion_cdc_full_delta_append_only_prod',
    default_args=default_args,
    description='CDC pipeline with APPEND direct load , FULL/DELTA CDC flow',
    schedule_interval='0 2 * * *',  # Run daily at 2 AM
    start_date=datetime(2026, 6, 1),
    catchup=False,
    max_active_runs=1,
    tags=['snowflake', 'cdc', 'ingestion',  'append','trnsct'],
) as dag:
    ...
    # =========================================================================
    # TASK 2: Extract APPEND Configurations
    # =========================================================================
    @task(task_id='get_append_configs')
    def get_append_configs(all_configs):
        """
        Filter and return only APPEND configurations.
        """
        append_configs = [cfg for cfg in all_configs if cfg['LOAD_MODE'] == 'APPEND']
        
        print(f"✓ Found {len(append_configs)} APPEND table(s):")
        for cfg in append_configs:
            print(f"  - {cfg['TABLE_NAME']} (APPEND)")
        
        return append_configs

    # =========================================================================
    # TASK 3: Extract CDC (FULL/DELTA) Configurations
    # =========================================================================
    ...
    # =========================================================================
    # TASK 4: Validate Configurations Exist
    # =========================================================================
    ...
    # =========================================================================
    # TASK 5: File Sensor Task (runs ONCE per schema, not per table)
    # =========================================================================
    ...
    # =========================================================================
    # TASK 6: Validate File Sensor Results (schema-level, runs once)
    # =========================================================================
    ...
    # =========================================================================
    # APPEND PATH: Direct Load to Target (Skip Staging/CDC/Refresh)
    # =========================================================================
    @task(task_id='load_append_direct')
    def load_append_direct(table_config, **context):
        """
        APPEND PATH: Load files directly from stage to target table.
        ...
        actual_file_pattern = table_config['FILE_PATTERN'].replace('{date}', '20260707')
    ...
    def cdc_process(load_result, **context):
        ...
        # Call sp_cdc_process 
        cdc_sql = f"""
        CALL {SP_SCHEMA}.sp_cdc_process_v2(
            '{run_id}',
            '{table_name}',
            '{target_schema}',
            '{staging_table}',
    ...
    # =========================================================================
    # TASK: Combine Results from Both Paths
    # =========================================================================
    @task(task_id='combine_results')
    def combine_results(append_results=None, cdc_results=None):
        """
        Combine results from APPEND and CDC paths into single list.
        Handles LazyXComSelectSequence from dynamic task mapping.
        """
        combined = []
        
        # Materialize append results (handles LazyXComSelectSequence)
        if append_results is not None:
            try:
                # Force evaluation of lazy sequence by iterating
                for item in append_results:
                    # Each item might itself be a lazy sequence - check and materialize
                    if hasattr(item, '__iter__') and not isinstance(item, (dict, str)):
                        # It's iterable (possibly nested lazy sequence)
                        try:
                            for result in item:
                                if result is not None:
                                    combined.append(result)
                        except:
                            # Not iterable or single result
                            if item is not None:
                                combined.append(item)
                    else:
                        # Simple dict or value
                        if item is not None:
                            combined.append(item)
                print(f"  APPEND: Added {len([r for r in combined if r.get('load_pattern') == 'APPEND'])} results")
            except Exception as e:
                print(f"  APPEND: Error processing results - {e}")
        
        # Materialize CDC results (handles LazyXComSelectSequence)
        if cdc_results is not None:
            try:
                # Force evaluation of lazy sequence by iterating
                for item in cdc_results:
                    ...
                    if hasattr(item, '__iter__') and not isinstance(item, (dict, str)):
                        ...
                        if item is not None:
                            combined.append(item)
                print(f"  CDC: Added {len([r for r in combined if r.get('load_pattern') == 'CDC'])} results")
            except Exception as e:
                print(f"  CDC: Error processing results - {e}")
        
        print(f"✓ Combined {len(combined)} total results from both paths")
        
        # Verify all items are dicts (JSON serializable)
        for idx, item in enumerate(combined):
            if not isinstance(item, dict):
                print(f"⚠️ Warning: Item {idx} is type {type(item)}, not dict")
        
        # Return simple list of dicts (JSON serializable)
        return combined

    # =========================================================================
    # TASK: Generate Summary Report (All Paths Converge Here)
    # =========================================================================
    @task(task_id='generate_summary')
    def generate_summary(all_results, **context):
        """
        Generate summary report from both APPEND and CDC paths.
        ...
        # Ensure all_results is iterable (handle edge cases)
        try:
        ...
        for result in results_list:
            ...
            if status == 'SUCCESS':
                ...
                if load_pattern == 'APPEND':
                    print(f"✓ {table_name}: {rows_loaded} loaded, {rows_inserted}I (APPEND direct)")
                else:
    ...
    # =========================================================================
    # Task Flow Definition - Conditional Branching
    # =========================================================================
    ...
    # 3. Split configs by load mode (APPEND vs CDC)
    append_configs = get_append_configs(all_configs)
    cdc_configs = get_cdc_configs(all_configs)
    
    # =========================================================================
    # SENSOR: Run file sensor ONCE for entire schema, then validate
    # =========================================================================
    schema_sensor_result = file_sensor_schema()
    validated_sensor = validate_file_sensor(schema_sensor_result)
    
    # =========================================================================
    # APPEND PATH: validated_sensor → load_append_direct (per table)
    # =========================================================================
    append_final_results = load_append_direct.expand(table_config=append_configs)
    ...
    # =========================================================================
    # CONVERGENCE: Combine both paths for summary
    # =========================================================================
    all_results = combine_results(append_final_results, cdc_final_results)
    summary = generate_summary(all_results)
    
    # Set dependencies
    all_configs >> validation
    validation >> [append_configs, cdc_configs, schema_sensor_result]
    schema_sensor_result >> validated_sensor
    [validated_sensor, append_configs] >> append_final_results
    [validated_sensor, cdc_configs] >> cdc_load_results >> cdc_process_results >> cdc_final_results
    [append_final_results, cdc_final_results] >> all_results >> summary