...
SOURCE_SCHEMA = 'AFE_REPORTING'
CONFIG_DB = 'DB_MAIN_CONSUMPTION_DEV'
CONFIG_SCHEMA = 'ACN_METADATA_CONFIG'
...
# =============================================================================
with DAG(
    dag_id='qdh-snowflake_afe_reporting_ingestion_append_only',
    ...
    catchup=False,
    max_active_runs=1,
    tags=['snowflake', 'append', 'ingestion'],
) as dag:
    ...
    def generate_summary(all_results, **context):
        ...
        # Ensure all_results is iterable
        try:
    ...
    # =========================================================================
    # Task Flow Definition - APPEND Only
    # =========================================================================
    ...
    summary = generate_summary(load_results)
    
    # Set dependencies
    append_configs >> validation >> schema_sensor_result >> validated_sensor
    [validated_sensor, append_configs] >> load_results >> summary