create or replace TABLE DB_MAIN_CONSUMPTION_DEV.ACN_METADATA_CONFIG.ETL_AUDIT_LOG (
	...
	PROCESSING_DATE DATE COMMENT 'Business date (e.g., 20260623)',
	SOURCE_SCHEMA VARCHAR(100) COMMENT 'e.g., IBIS',
	TABLE_NAME VARCHAR(255),
	TARGET_SCHEMA VARCHAR(255),
	STAGE_NAME VARCHAR(255),
	PROCEDURE_NAME VARCHAR(255) COMMENT ' sp_load_to_staging, sp_cdc_process, sp_refresh_lv, sp_load_append',
	JOB_STATUS VARCHAR(50) NOT NULL COMMENT 'STARTED, SUCCESS, FAILED, SKIPPED',
	JOB_START_TIME TIMESTAMP_NTZ(9) NOT NULL,
	JOB_END_TIME TIMESTAMP_NTZ(9),
	SOURCE_ROW_COUNT NUMBER(38,0) COMMENT 'Rows in staging table',
	TARGET_ROW_COUNT NUMBER(38,0) COMMENT 'Rows in target table after processing',
	ROWS_INSERTED NUMBER(38,0) COMMENT 'CDC inserts',
	ROWS_UPDATED NUMBER(38,0) COMMENT 'CDC updates',
	ROWS_DELETED NUMBER(38,0) COMMENT 'CDC deletes',
	FILES_LOADED NUMBER(38,0) COMMENT 'Files successfully loaded',
	ERROR_MESSAGE VARCHAR(16777216),
	CREATED_BY VARCHAR(100) DEFAULT CURRENT_USER(),
	CREATED_TIMESTAMP TIMESTAMP_NTZ(9) DEFAULT CURRENT_TIMESTAMP(),
	primary key (AUDIT_ID)
);
create or replace TABLE DB_MAIN_CONSUMPTION_DEV.ACN_METADATA_CONFIG.ETL_INGEST_CONFIG (
	...
	FILE_PATTERN VARCHAR(16777216),
	FIELD_DELIMITER VARCHAR(16777216),
	SKIP_HEADER NUMBER(38,0) DEFAULT 1,
	ON_ERROR VARCHAR(16777216) DEFAULT 'ABORT_STATEMENT',
	LOAD_MODE VARCHAR(16777216) NOT NULL,
	MERGE_KEYS VARCHAR(16777216),
	IS_ACTIVE BOOLEAN DEFAULT TRUE,
	PRIORITY NUMBER(38,0) DEFAULT 100,
	BATCH_GROUP NUMBER(38,0) DEFAULT 1,
	WAREHOUSE_NAME VARCHAR(16777216),
	STAGING_TABLE VARCHAR(16777216),
	NOTES VARCHAR(16777216),
	FILECHECK_IS_MANDATORY BOOLEAN DEFAULT TRUE,
	CREATED_AT TIMESTAMP_NTZ(9) DEFAULT CURRENT_TIMESTAMP(),
	UPDATED_AT TIMESTAMP_NTZ(9) DEFAULT CURRENT_TIMESTAMP(),
	constraint UK_INGEST_CONFIG unique (SOURCE_SCHEMA, TABLE_NAME),
	constraint PK_INGEST_CONFIG primary key (CONFIG_ID)
);
...
def cdc_process(session, run_id, table_name, target_schema, staging_table_fqn, merge_keys, config_schema, load_pattern=''FULL'',
    ...
    try:
        ...
        raise
';
CREATE OR REPLACE PROCEDURE DB_MAIN_CONSUMPTION_DEV.ACN_METADATA_CONFIG.SP_CDC_PROCESS_V2("RUN_ID" VARCHAR, "TABLE_NAME" VARCHAR, "TARGET_SCHEMA" VARCHAR, "STAGING_TABLE_FQN" VARCHAR, "MERGE_KEYS" VARCHAR, "CONFIG_SCHEMA" VARCHAR, "LOAD_PATTERN" VARCHAR DEFAULT 'FULL', "DAG_NAME" VARCHAR DEFAULT null, "TASK_NAME" VARCHAR DEFAULT null, "PROCESSING_DATE" DATE DEFAULT null)
RETURNS VARIANT
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
HANDLER = 'cdc_process_v2'
EXECUTE AS CALLER
AS '
import json
import logging
...
def cdc_process_v2(session, run_id, table_name, target_schema, staging_table_fqn, merge_keys, config_schema, load_pattern=''FULL'',
               dag_name=None, task_name=None, processing_date=None):
    """
    CDC processing: Compare staging vs _LV, detect I/U/D, append to base table.
    Uses separate audit SPs for logging.
    ...
    try:
        ...
        # Exclude ROW_HASH from insert columns (computed dynamically, not stored)
        insert_columns_list = [col for col in all_columns if col != ''ROW_HASH'']
        ...
        hash_columns = ", ".join([f"stg.{col}" for col in data_columns])
        lv_hash_columns = ", ".join([f"lv.{col}" for col in data_columns])
        merge_key_join = " AND ".join([f"lv.{key} = stg.{key}" for key in merge_key_list])
        ...
        if not lv_exists or lv_row_count == 0:
            ...
            if load_pattern == ''FULL'':
                ...
                (lv.{merge_key} IS NULL)
                OR (stg.{merge_key} IS NULL AND lv.HDP_DML_CODE != ''D'')
                OR (stg.calculated_hash IS NOT NULL AND lv.calculated_lv_hash IS NOT NULL AND stg.calculated_hash != lv.calculated_lv_hash)
                """
            elif load_pattern == ''DELTA'':
                where_clause = """
                (lv.{merge_key} IS NULL)
                OR (stg.calculated_hash IS NOT NULL AND lv.calculated_lv_hash IS NOT NULL AND stg.calculated_hash != lv.calculated_lv_hash AND stg.{merge_key} IS NOT NULL)
                """
            elif load_pattern == ''APPEND'':
                where_clause = """
                (lv.{merge_key} IS NULL)
            ...
            if load_pattern == ''FULL'':
                dml_code_logic = f"""
                    CASE
                        WHEN lv.{merge_key_list[0]} IS NULL THEN ''I''
                        WHEN stg.{merge_key_list[0]} IS NULL THEN ''D''
                        WHEN stg.calculated_hash != lv.calculated_lv_hash THEN ''U''
                        ELSE NULL
                    END AS HDP_DML_CODE"""
            elif load_pattern == ''DELTA'':
                dml_code_logic = f"""
                    CASE
                        WHEN lv.{merge_key_list[0]} IS NULL THEN ''I''
                        WHEN stg.{merge_key_list[0]} IS NOT NULL AND stg.calculated_hash != lv.calculated_lv_hash THEN ''U''
                        ELSE NULL
                    END AS HDP_DML_CODE"""
            elif load_pattern == ''APPEND'':
                dml_code_logic = f"""
            ...
            lv_with_hash AS (
                SELECT *,
                    HASH({lv_hash_columns}) AS calculated_lv_hash
                FROM {lv_table} lv
            ),
            cdc_full_outer AS (
                SELECT
                    ...
                    {dml_code_logic}
                FROM staging_with_hash stg
                FULL OUTER JOIN lv_with_hash lv
                    ON {merge_key_join}
                WHERE {where_clause}
            )
            SELECT * FROM cdc_full_outer
...
def load_append(session, run_id, target_table_fqn, stage_name, file_format, 
    ...
    try:
        ...
        raise
';
CREATE OR REPLACE PROCEDURE DB_MAIN_CONSUMPTION_DEV.ACN_METADATA_CONFIG.SP_LOAD_APPEND_V2("RUN_ID" VARCHAR, "TARGET_TABLE_FQN" VARCHAR, "STAGE_NAME" VARCHAR, "FILE_FORMAT" VARCHAR, "FILE_PATTERN" VARCHAR, "ON_ERROR" VARCHAR, "MERGE_KEYS" VARCHAR, "DAG_NAME" VARCHAR DEFAULT null, "TASK_NAME" VARCHAR DEFAULT null, "SRC_SCHEMA" VARCHAR DEFAULT null, "PROCESSING_DATE" DATE DEFAULT null)
RETURNS VARIANT
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
HANDLER = 'load_append_v2'
EXECUTE AS CALLER
AS '
import json
import logging
...
def load_append_v2(session, run_id, target_table_fqn, stage_name, file_format, 
                       file_pattern, on_error, merge_keys, dag_name, task_name, 
                       src_schema, processing_date):
    """
    APPEND-only pattern: Load CSV files directly from stage to target table.
    Uses MATCH_BY_COLUMN_NAME from file format to handle varying column positions.
    Uses separate audit SPs for logging.
    """
    ...
    # MAIN LOGIC - Direct Load to Target using MATCH_BY_COLUMN_NAME
    try:
        pattern_sql = f"PATTERN = ''{file_pattern}''" if file_pattern and file_pattern != ''.*'' else ""
        ...
        # Identify audit/metadata columns in target table
        desc_result = session.sql(f"DESC TABLE {target_table_fqn}").collect()
        
        audit_columns = []
        filename_column = None
        filename_variants = (''FILENAME'', ''HDP_FILENAME'', ''FILE_NAME'')
        metadata_columns = [''HDP_LAST_UPDT_TSTAMP'', ''HDP_LAST_UPDT_USER'', ''HDP_DML_CODE'']
        ...
        for row in desc_result:
            ...
            if col_name in metadata_columns:
                audit_columns.append(col_name)
        
        # Step 1: COPY INTO using MATCH_BY_COLUMN_NAME (no transform)
        # INCLUDE_METADATA populates the filename column from file metadata
        include_metadata_sql = f"INCLUDE_METADATA = ({filename_column} = METADATA$FILENAME)" if filename_column else ""
        ...
        {pattern_sql}
        ON_ERROR = {on_error}
        {include_metadata_sql}
        """
        
        log.info(f"[sp_load_append] Executing COPY with MATCH_BY_COLUMN_NAME...")
        ...
        log.info(f"[sp_load_append] Loaded {rows_loaded} rows from {files_ok} files")
        
        # Step 2: UPDATE audit columns for newly loaded rows (where audit cols are NULL)
        if rows_loaded > 0 and audit_columns:
            set_parts = []
            if ''HDP_LAST_UPDT_TSTAMP'' in audit_columns:
                set_parts.append("HDP_LAST_UPDT_TSTAMP = CURRENT_TIMESTAMP()")
            if ''HDP_LAST_UPDT_USER'' in audit_columns:
                set_parts.append("HDP_LAST_UPDT_USER = ''INFORMATICA''")
            if ''HDP_DML_CODE'' in audit_columns:
                set_parts.append("HDP_DML_CODE = ''I''")
            
            if set_parts:
                set_clause = '', ''.join(set_parts)
                update_sql = f"""
                UPDATE {target_table_fqn}
                SET {set_clause}
                WHERE HDP_DML_CODE IS NULL
                """
                session.sql(update_sql).collect()
                log.info(f"[sp_load_append] Updated audit columns for loaded rows")
